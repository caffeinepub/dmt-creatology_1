...
import {
  useAllTransportBookings,
  useUpdateTransportBookingStatus,
} from "@/hooks/useAdminQueries";
import { Bus, Car, IndianRupee, Plane, Ship, Train } from "lucide-react";
import { BookingStatus, TransactionStatus } from "../../backend.d";
import type { TransportBooking } from "../../backend.d";
...
function BookingStatusSelect({ booking }: { booking: TransportBooking }) {
  const updateStatus = useUpdateTransportBookingStatus();
  ...
  return (
    <Select value={booking.status} onValueChange={handleChange}>
      <SelectTrigger
        ...
        data-ocid="transport-bookings.status.select"
      >
        <SelectValue />
      </SelectTrigger>
...
function BookingsTable({
  ...
  bookings: TransportBooking[] | undefined;
  isLoading: boolean;
}) {
  if (isLoading) {
    return (
      <div
        ...
        data-ocid="transport-bookings.table.loading_state"
      >
        {[1, 2, 3, 4, 5].map((k) => (
  ...
  if (!bookings?.length) {
    return (
      <div
        ...
        data-ocid="transport-bookings.table.empty_state"
      >
        ...
        <p className="text-slate-600 text-sm mt-1">
          Transport booking records will appear here after passengers complete
          payment.
        </p>
      </div>
    );
  ...
  return (
    <div className="overflow-x-auto">
      <Table data-ocid="transport-bookings.table">
        <TableHeader>
        ...
        <TableBody>
          {bookings.map((booking, index) => {
            const Icon = getTransportTypeIcon(booking.transportType);
            return (
              <TableRow
                key={String(booking.id)}
                className="border-slate-800 hover:bg-slate-800/40 transition-colors"
                data-ocid={`transport-bookings.table.row.${index + 1}`}
              >
                <TableCell>
                  <div>
                ...
                <TableCell>
                  <span className="text-slate-300 text-sm">
                    {formatNanoDate(booking.travelDate)}
                  </span>
                </TableCell>
                <TableCell>
                  <span className="text-slate-400 text-sm">
...
export default function AdminTransportBookingsPage() {
  const { data: bookings, isLoading } = useAllTransportBookings();
  ...
  return (
    <div className="p-4 md:p-6 space-y-6">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-0.5">
            All transport reservation records
          </p>
        </div>
      ...
      <div className="bg-slate-900 rounded-2xl border border-slate-800 overflow-hidden">
        <Tabs defaultValue="all" data-ocid="transport-bookings.filter.tab">
          <div className="px-4 pt-4 border-b border-slate-800">
            <TabsList className="bg-slate-800/60 border-0">
              <TabsTrigger
                ...
                data-ocid="transport-bookings.all.tab"
              >
              ...
              <TabsTrigger
                ...
                data-ocid="transport-bookings.paid.tab"
              >
                Paid ({paid?.length ?? 0})
              </TabsTrigger>
              <TabsTrigger
                ...
                data-ocid="transport-bookings.pending.tab"
              >
