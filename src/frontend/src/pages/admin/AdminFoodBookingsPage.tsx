...
import type { FoodBooking } from "../../backend.d";
...
function BookingStatusSelect({ booking }: { booking: FoodBooking }) {
  const updateStatus = useUpdateFoodBookingStatus();
  ...
  return (
    <Select value={booking.status} onValueChange={handleChange}>
      <SelectTrigger
        ...
        data-ocid="admin-food-bookings.status.select.1"
      >
        <SelectValue />
      </SelectTrigger>
...
function BookingsTable({
  ...
  bookings: FoodBooking[] | undefined;
  isLoading: boolean;
}) {
  if (isLoading) {
    return (
      <div
        ...
        data-ocid="admin-food-bookings.loading_state"
      >
        {[1, 2, 3, 4, 5].map((k) => (
  ...
  if (!bookings?.length) {
    return (
      <div
        ...
        data-ocid="admin-food-bookings.empty_state"
      >
        <div className="w-14 h-14 rounded-full bg-slate-800 flex items-center justify-center mb-4">
          <UtensilsCrossed size={24} className="text-slate-600" />
        </div>
        <p className="text-slate-400 font-medium">No food bookings found</p>
        <p className="text-slate-600 text-sm mt-1">
          Food booking records will appear here after guests complete payment.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="overflow-x-auto">
      <Table data-ocid="admin-food-bookings.table">
        <TableHeader>
          <TableRow className="border-slate-800 hover:bg-transparent">
            <TableHead className="text-slate-400 text-xs uppercase tracking-wider font-medium min-w-[150px]">
              Caterer
            </TableHead>
        ...
        <TableBody>
          {bookings.map((booking, index) => (
            <TableRow
              ...
              data-ocid={`admin-food-bookings.item.${index + 1}`}
            >
              <TableCell>
                <div>
...
export default function AdminFoodBookingsPage() {
  const { data: bookings, isLoading } = useAllFoodBookings();
  ...
  return (
    <div className="p-4 md:p-6 space-y-6" data-ocid="admin-food-bookings.page">
      {/* Header */}
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-0.5">
            All catering reservation records
          </p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <div className="bg-slate-800/40 rounded-xl border border-slate-700/50 p-4 text-center">
          ...
          <p className="text-slate-500 text-xs mt-0.5">Paid</p>
        </div>
        <div className="bg-slate-800/40 rounded-xl border border-slate-700/50 p-4 text-center">
          <p className="text-2xl font-bold text-red-400">
            {cancelled?.length ?? 0}
          </p>
          <p className="text-slate-500 text-xs mt-0.5">Cancelled</p>
        </div>
      </div>
      ...
      <div className="bg-slate-900 rounded-2xl border border-slate-800 overflow-hidden">
        <Tabs defaultValue="all">
          <div className="px-4 pt-4 border-b border-slate-800">
            <TabsList className="bg-slate-800/60 border-0">
              <TabsTrigger
                ...
                data-ocid="admin-food-bookings.all.tab"
              >
              ...
              <TabsTrigger
                ...
                data-ocid="admin-food-bookings.paid.tab"
              >
                Paid ({paid?.length ?? 0})
              </TabsTrigger>
              <TabsTrigger
                ...
                data-ocid="admin-food-bookings.pending.tab"
              >
                Pending ({pending?.length ?? 0})
              </TabsTrigger>
              <TabsTrigger
                value="cancelled"
                className="data-[state=active]:bg-red-600 data-[state=active]:text-white text-slate-400 text-xs"
                data-ocid="admin-food-bookings.cancelled.tab"
              >
          ...
          <TabsContent value="cancelled" className="mt-0">
            <BookingsTable bookings={cancelled} isLoading={isLoading} />
          </TabsContent>
        </Tabs>
      </div>
    </div>
